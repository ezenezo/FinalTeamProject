			...
			<table class="table">
				...
				<tbody>
					...
					<c:forEach items="${list_c}" var="cvo">
						<tr>
							<td>${cvo.avo.apprNo}</td>
							<td>${cvo.clist[0].clubNm}</td>
							<td>${cvo.avo.empNo}</td>
							<td>${cvo.avo.readOrNot}</td>
				...
				<ul class="pagination">
					...
					<c:forEach begin="${ph_c.startPage}" end="${ph_c.endPage }" var="i">
						<li class="page-item"><a class="page-link"
							href="/approval/list?pageNo=${i } &qty=${ph_c.pgvo.qty}&type=${ph_c.pgvo.type}&keyword=${ph_c.pgvo.keyword}">${ i}</a></li>
					</c:forEach>
					<!-- 다음 -->
